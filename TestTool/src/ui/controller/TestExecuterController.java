...
public class TestExecuterController {
	...
	public void initialize() {
		...
		lvTestFiles.setContextMenu(getTestFileListContextMenu());
	...
	private ContextMenu getTestFileListContextMenu() {
		MenuItem miAddFile = new MenuItem("Dateien hinzufügn");
		miAddFile.setOnAction(event -> browseTestFiles(null));

		MenuItem miAddDir = new MenuItem("Ordner hinzufügn");
		miAddDir.setOnAction(event -> browseTestDirectory(null));

		MenuItem miRem = new MenuItem("Entfernen");
		miRem.setOnAction(event -> removeSelectedFile());
		miRem.disableProperty().bind(lvTestFiles.getSelectionModel().selectedItemProperty().isNull());

		MenuItem miClear = new MenuItem("Alles Entfern");
		miClear.setOnAction(event -> selectedFiles.clear());
		miClear.disableProperty().bind(selectedFiles.sizeProperty().isEqualTo(0));

		ContextMenu menu = new ContextMenu(miAddFile, miAddDir, miRem, miClear);
		return menu;
	}
	...
	private void finishedExecution(TestSuiteProtocol protocol) {
		...
		if (protocol != null) {
			...
			try {
				...
				Platform.runLater(() -> MainApplication.showAlert("Protokoll konnte nicht gespeichert werden",
						"Erstellen Sie eine Protokolldatei manuel"));
			}
		} else {
			taOutput.clear();
		}
